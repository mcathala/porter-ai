    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] bg-[#111a22]/80 backdrop-blur-sm transition-all duration-300">
            ...
            <div className="w-full max-w-2xl mx-4 transform transition-all relative z-10 flex flex-col gap-4">
                <div className="relative overflow-hidden rounded-2xl bg-[#1a2632] shadow-2xl ring-1 ring-white/10">
                    ...
                    <div className="relative flex items-center gap-4 px-6 py-6">
                        ...
                        <button
                            onClick={onClose}
                            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition-colors"
                            title="Close (Esc)"
                        >
                        ...
                        <div className="flex flex-wrap gap-2">
                            {suggestedActions.map((action) => (
                                <button
                                    ...
                                    className="flex items-center gap-2 rounded-full border border-[#233648] bg-[#1a2632] px-4 py-2 text-sm font-medium text-slate-300 hover:border-primary hover:text-white transition-colors shadow-sm cursor-pointer"
                                >
                    ...
                    <div className="flex items-center justify-between px-4 py-2 bg-[#0d141b] border-t border-[#233648] text-[10px] text-gray-500">
                        <div className="flex gap-4">
                            <span className="flex items-center gap-1">
                                <kbd className="font-sans px-1.5 py-0.5 rounded bg-[#1a2632] border border-[#233648]">
                                    ↵
                                </kbd>{" "}
                                to select
                            </span>
                        ...
                        <button
                            onClick={onNextTurn}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#1a2632] border border-[#233648] hover:border-white/20 text-gray-400 hover:text-white text-xs font-medium transition-colors cursor-pointer"
                        >
                ...
                {takenActions.length > 0 && (
                    <div className="w-full">
                        <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2 pl-2">
                            Taken Actions
                        </h4>
                        <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-700">
                            {takenActions.map((action) => (
                                <div
                                    key={action.id}
                                    className="group bg-[#1a2632]/80 backdrop-blur-sm rounded-xl p-4 border border-[#233648] shadow-lg flex items-center justify-between animate-in fade-in slide-in-from-top-2 duration-300 hover:border-primary/30 transition-colors"
                                >
